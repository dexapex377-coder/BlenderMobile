...
void MorphologicalDistanceFeatherWeights::compute_weights(int radius)
{
  ...
  weights_texture_ = GPU_texture_create_2d(
      "Weights", size,1, 1, GPU_R16F, GPU_TEXTURE_USAGE_GENERAL, weights.data());
}
...
void MorphologicalDistanceFeatherWeights::compute_distance_falloffs(int type, int radius)
{
  ...
  distance_falloffs_texture_ = GPU_texture_create_2d(
      "Distance Factors", size,1, 1, GPU_R16F, GPU_TEXTURE_USAGE_GENERAL, falloffs.data());
}
