...
static void standard_defines(Vector<const char *> &sources)
{
  ...
  /* Mobile GPU detection via vendor string fallback */
  else {
    const char *_vendor = GPU_platform_vendor();
    const char *_renderer = GPU_platform_renderer();
    if (_vendor && (strstr(_vendor, "ARM") || strstr(_renderer, "Mali") ||
                    strstr(_vendor, "Mali"))) {
      sources.append("#define GPU_MALI\n");
    }
    else if (_vendor && (strstr(_vendor, "Qualcomm") || strstr(_renderer, "Adreno") ||
                         strstr(_vendor, "QCOM"))) {
      sources.append("#define GPU_ADRENO\n");
    }
    else if (_vendor && (strstr(_vendor, "Imagination") || strstr(_renderer, "PowerVR") ||
                         strstr(_vendor, "PowerVR"))) {
      sources.append("#define GPU_POWERVR\n");
    }
  }
