...
void main()
{
  ...
  if (ViewportSize.x > 0.0) {
    ...
    vec2 px_pos = (gl_Position.xy + 1.0) * scale;
    vec2 adj_pos = round(px_pos - 0.5) + 0.5;
    gl_Position.xy = adj_pos / scale - 1.0;
  }
  ...
  float half_width = (0.06 + (size - 10.0) * 0.04);
  ...
  gl_PointSize = ceil(ext_radius + thresholds.y) * 2.0 + 1.0;
