...
static char *glsl_patch_default_get(bool hasLayoutIndex)
{
  /** Used for shader patching. Init once. */
    static char patch1[4096] = "\0";
    static char patch2[4096] = "\0";
    if(hasLayoutIndex){
        if (patch2[0] != '\0') {
            return patch2;
  }
    ...
    char patch[4096] = "\0";
  size_t slen = 0;
  /* Version need to go first. Detect GLES vs desktop GL. */
  int _gl_ver = epoxy_gl_version();
  if (epoxy_is_gles()) {
    if (_gl_ver >= 32) {
      STR_CONCAT(patch, slen, "#version 320 es\n");
    } else if (_gl_ver >= 31) {
      STR_CONCAT(patch, slen, "#version 310 es\n");
    } else {
      STR_CONCAT(patch, slen, "#version 300 es\n");
    }
  } else {
    if (_gl_ver >= 43) {
      ...
      STR_CONCAT(patch, slen, "#version 150\n");
    }
  }
  ...
  if (epoxy_is_gles()) {
    ...
    if (_gl_ver >= 32) {
      ...
      STR_CONCAT(patch, slen, "#extension GL_EXT_gpu_shader5 : enable\n");
      STR_CONCAT(patch, slen, "#define GPU_ARB_gpu_shader5\n");
    } else if (_gl_ver >= 31) {
      /* GLES 3.1 has geometry shaders, texture buffer and gpu_shader5 via extensions */
      if (epoxy_has_gl_extension("GL_EXT_geometry_shader")) {
        STR_CONCAT(patch, slen, "#extension GL_EXT_geometry_shader : enable\n");
      }
      if (epoxy_has_gl_extension("GL_EXT_texture_buffer")) {
        STR_CONCAT(patch, slen, "#extension GL_EXT_texture_buffer : enable\n");
      }
      if (epoxy_has_gl_extension("GL_EXT_gpu_shader5")) {
        STR_CONCAT(patch, slen, "#extension GL_EXT_gpu_shader5 : enable\n");
        STR_CONCAT(patch, slen, "#define GPU_ARB_gpu_shader5\n");
      }
      STR_CONCAT(patch, slen, "#extension GL_EXT_shader_io_blocks : enable\n");
    ...
    if (GLContext::texture_gather_support) {
      STR_CONCAT(patch, slen, "#extension GL_EXT_texture_gather : enable\n");
      STR_CONCAT(patch, slen, "#define GPU_ARB_texture_gather\n");
    }
    if (GLContext::shader_draw_parameters_support) {
      STR_CONCAT(patch, slen, "#extension GL_EXT_base_instance : enable\n");
      STR_CONCAT(patch, slen, "#define gpu_BaseInstance gl_BaseInstance\n");
    }
    if (epoxy_has_gl_extension("GL_EXT_conservative_depth")) {
      STR_CONCAT(patch, slen, "#extension GL_EXT_conservative_depth : enable\n");
    }
    if (GLContext::clip_cull_distance_support){
      STR_CONCAT(patch, slen, "#extension GL_EXT_clip_cull_distance : enable\n");
    }
    if (hasLayoutIndex && _gl_ver >= 32) {
  ...
  if (GPU_shader_image_load_store_support()) {
    /* image_load_store is enabled via GLES 3.x core or desktop extension */
  }

  /* GLES precision qualifiers (safe on desktop GL too, ignored) */
  STR_CONCAT(patch, slen, "precision highp int;\n");
  STR_CONCAT(patch, slen, "precision highp float;\n");
  STR_CONCAT(patch, slen, "precision highp sampler2D;\n");
  STR_CONCAT(patch, slen, "precision highp samplerCube;\n");
  STR_CONCAT(patch, slen, "precision highp sampler2DArray;\n");
  STR_CONCAT(patch, slen, "precision highp sampler2DShadow;\n");
  ...
  STR_CONCAT(patch, slen, "precision highp isampler2D;\n");
  ...
  STR_CONCAT(patch, slen, "precision highp isamplerBuffer;\n");
  ...
  BLI_assert(slen < sizeof(patch));
    if(hasLayoutIndex){
        ::memcpy(patch2,patch,sizeof(patch2));
    }else{
        ::memcpy(patch1,patch,sizeof(patch1));
    }
    if(hasLayoutIndex){
        return patch2;
    }else{
...
static char *glsl_patch_compute_get()
{
  ...
  if (epoxy_is_gles()) {
    ...
    /* GLES compute is core in 3.1+, no extension needed */
    if (_gl_ver >= 32) {
      STR_CONCAT(patch, slen, "#extension GL_EXT_geometry_shader : enable\n");
      STR_CONCAT(patch, slen, "#extension GL_EXT_shader_io_blocks : enable\n");
      STR_CONCAT(patch, slen, "#extension GL_EXT_texture_buffer : enable\n");
  ...
  /* GLES precision qualifiers (safe on desktop too) */
  STR_CONCAT(patch, slen, "precision highp int;\n");
  STR_CONCAT(patch, slen, "precision highp float;\n");
  STR_CONCAT(patch, slen, "precision highp sampler2D;\n");
  STR_CONCAT(patch, slen, "precision highp samplerCube;\n");
