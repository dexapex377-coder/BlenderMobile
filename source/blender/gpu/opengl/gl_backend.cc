...
void GLBackend::platform_init()
{
  ...
  /* --- Mobile GPU detection (Android) --- */
  else if (strstr(vendor, "Qualcomm") || strstr(renderer, "Adreno") ||
           strstr(vendor, "QCOM")) {
    device = GPU_DEVICE_UNKNOWN;  /* Treat as UNKNOWN but we know it's Adreno */
    driver = GPU_DRIVER_OFFICIAL;
    printf("Detected Qualcomm/Adreno GPU\n");
  }
  else if (strstr(vendor, "ARM") || strstr(renderer, "Mali") ||
           strstr(vendor, "Mali")) {
    device = GPU_DEVICE_UNKNOWN;
    driver = GPU_DRIVER_OFFICIAL;
    printf("Detected ARM Mali GPU\n");
  }
  else if (strstr(vendor, "Imagination") || strstr(renderer, "PowerVR") ||
           strstr(vendor, "PowerVR")) {
    device = GPU_DEVICE_UNKNOWN;
    driver = GPU_DRIVER_OFFICIAL;
    printf("Detected Imagination PowerVR GPU\n");
  }
  else if (strstr(vendor, "MediaTek") || strstr(renderer, "MediaTek") ||
           strstr(vendor, "MTK")) {
    device = GPU_DEVICE_UNKNOWN;
    driver = GPU_DRIVER_OFFICIAL;
    printf("Detected MediaTek GPU\n");
  }
  else if (strstr(vendor, "Broadcom") || strstr(renderer, "Broadcom") ||
           strstr(vendor, "VideoCore") || strstr(renderer, "VideoCore")) {
    device = GPU_DEVICE_UNKNOWN;
    driver = GPU_DRIVER_OFFICIAL;
    printf("Detected Broadcom VideoCore GPU\n");
  }
  else {
...
void GLBackend::capabilities_init()
{
  ...
  if (epoxy_is_gles()) {
    GCaps.shader_image_load_store_support = (epoxy_gl_version() >= 31);
  } else {
  ...
  if (epoxy_is_gles()) {
    /* GLES 3.1+ has compute shaders in core. GLES 3.0 does not. */
    GCaps.compute_shader_support = (epoxy_gl_version() >= 31);
  } else {
    GCaps.compute_shader_support = (epoxy_gl_version() >= 43);
  }
  ...
  if (epoxy_is_gles()) {
    GCaps.shader_storage_buffer_objects_support = (epoxy_gl_version() >= 31);
  } else {
  ...
  if (epoxy_is_gles()) {
    /* GLES 3.0+ supports layout(location = N) for vertex attributes. */
    GLContext::explicit_location_support = (epoxy_gl_version() >= 30);
  } else {
    GLContext::explicit_location_support = (epoxy_gl_version() >= 33);
  }
  if (epoxy_is_gles()) {
    GLContext::geometry_shader_invocations = (epoxy_gl_version() >= 32) ||
                                             epoxy_has_gl_extension("GL_EXT_gpu_shader5");
  } else {
  ...
  if (epoxy_is_gles()) {
    /* GLES 3.2 supports gl_Layer in core with geometry shaders.
     * GLES 3.1 may support it via GL_EXT_geometry_shader. */
    GLContext::layered_rendering_support = (epoxy_gl_version() >= 32) ||
                                           epoxy_has_gl_extension("GL_EXT_geometry_shader");
    /* Barycentric coordinates via GL_EXT_gpu_shader5 on GLES */
  ...
  if (epoxy_is_gles()) {
    GLContext::stencil_texturing_support = (epoxy_gl_version() >= 30);
  } else {
  ...
  if (epoxy_is_gles()) {
    /* GLES 3.0+ has textureGather() built-in, but extension check for older/GL_EXT_texture_gather */
    GLContext::texture_gather_support = (epoxy_gl_version() >= 30) ||
                                        epoxy_has_gl_extension("GL_EXT_texture_gather");
  } else {
  ...
  GLContext::vertex_attrib_binding_support = epoxy_has_gl_extension(
    ...
    if (epoxy_is_gles()) {
      /* GLES 3.0+ has gl_ClipDistance built-in. Also check for EXT_clip_cull_distance. */
      GLContext::clip_cull_distance_support = epoxy_has_gl_extension("GL_EXT_clip_cull_distance") ||
                                              (epoxy_gl_version() >= 30);
    } else {
      GLContext::clip_cull_distance_support = epoxy_has_gl_extension("GL_EXT_clip_cull_distance");
    }
