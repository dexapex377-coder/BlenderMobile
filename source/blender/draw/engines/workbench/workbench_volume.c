...
void workbench_volume_engine_init(WORKBENCH_Data *vedata)
{
  ...
  if (txl->dummy_volume_tx == NULL) {
    ...
    txl->dummy_volume_tx = GPU_texture_create_3d(
        "dummy_volume", 1, 1, 1, 1, GPU_RGBA16F, usage, zero);
    txl->dummy_shadow_tx = GPU_texture_create_3d(
        "dummy_shadow", 1, 1, 1, 1, GPU_RGBA16F, usage, one);
    txl->dummy_coba_tx = GPU_texture_create_2d("dummy_coba", 1,1, 1, GPU_RGBA16F, usage, zero);
  }
}
