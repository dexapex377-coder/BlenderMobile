...
void DRW_globals_update(void)
{
  ...
  if (!G_draw.ramp) {
    ...
    G_draw.ramp = GPU_texture_create_2d(
        "ramp", col_size,1, 1, GPU_RGBA8, GPU_TEXTURE_USAGE_SHADER_READ, colors);
...
static GPUTexture *DRW_create_weight_colorramp_texture(void)
{
  ...
  return GPU_texture_create_2d(
      "weight_color_ramp", 256,1, 1, GPU_RGBA32F, GPU_TEXTURE_USAGE_SHADER_READ, pixels[0]);
}
